...
- **container_network_type**: Container network type, assigned by referencing input variable `container_network_type`, default is "eni" for ENI network mode (Turbo clusters must use ENI network)
...
- [CCE Turbo Cluster Best Practice Source Code Reference](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/cce/turbo-cluster)
