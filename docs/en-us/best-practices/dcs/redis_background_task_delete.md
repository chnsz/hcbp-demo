# Deploy Redis Background Task Delete

## Application Scenario

Distributed Cache Service (DCS) is a high-performance, highly available in-memory database service provided by Huawei Cloud, supporting mainstream cache engines such as Redis and Memcached. During routine O&M of DCS instances, the system automatically generates background tasks during operations such as instance creation, scaling, configuration changes, backup, and restoration, and these tasks are recorded in the instance's background task list.

This best practice will introduce how to use Terraform to delete a specified background task of a DCS Redis instance. This practice operates on a pre-existing DCS instance and background task, and does not create any new infrastructure. It is suitable for scenarios where you need to clean up records of completed or abnormal background tasks.
...
- [DCS Background Task Delete (huaweicloud_dcs_background_task_delete)](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/dcs_background_task_delete)
...
2. Run `terraform plan` to view the resource deletion plan
3. After confirming the resource plan is correct, run `terraform apply` to start deleting the background task
4. Run `terraform show` to view the deleted background task information

## Reference Information

- [Huawei Cloud DCS Product Documentation](https://support.huaweicloud.com/dcs/index.html)
- [Huawei Cloud Provider Documentation](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [Best Practice Source Code Reference For DCS Redis Background Task Delete](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/dcs/redis-background-task-delete)
