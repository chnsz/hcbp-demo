...
This best practice will introduce how to use Terraform to deploy a DIS stream and configure parameters such as auto scaling, data format, and compression format to meet the data ingestion requirements of different business scenarios.
...
```hcl
# Create a DIS stream in the specified region (if the region parameter is omitted, it inherits the region specified in the current provider block)
...
- **retention_period**: Assigned by referencing the input variable stream_retention_period, used to specify the data retention period in hours, ranging from 24 to 72.
...
3. After confirming the resource plan is correct, run `terraform apply` to start creating the DIS stream
