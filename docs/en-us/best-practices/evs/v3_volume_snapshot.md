...
Elastic Volume Service (EVS) is a high-performance, highly reliable, and scalable block storage service provided by Huawei Cloud, offering persistent storage for computing resources such as Elastic Cloud Servers (ECS). An EVS snapshot backs up volume data at a specific point in time, so that data can be quickly restored when accidental deletion, data corruption, or service failures occur, ensuring business continuity.

This best practice will introduce how to use Terraform to automatically deploy an EVS V3 volume and a V3 snapshot, including querying availability zones and images, creating the volume, and creating a snapshot based on that volume.
...
Prepare a TF file (such as main.tf) in the specified workspace for writing the scripts of the current best practice, and ensure that it (or other TF files in the same directory) contains the provider version declaration and Huawei Cloud authentication information required for deploying resources.
...
Add the following script to the TF file (such as main.tf) to query the availability zones in the current region:
...
- This data source requires no additional parameters and queries all availability zones in the current region by default

### 3. Query the Images

Add the following script to the TF file (such as main.tf) to query the image used to create the volume:
...
**Parameter description**:
- **count**: Creates the data source when no image ID is specified, determined by referencing the input variable volume_image_id
...
- **volume_type**: The type of the volume, assigned by referencing the input variable volume_type, supporting SATA, SAS, GPSSD, SSD, ESSD, GPSSD2, and ESSD2
- **availability_zone**: The availability zone of the volume, using the first availability zone in the availability zone list by default when not specified
- **description**: The description of the volume, assigned by referencing the input variable volume_description
- **image_id**: The ID of the image used to create the volume, using the first image in the image list when not specified
...
- **size**: The size of the volume in GB, assigned by referencing the input variable volume_size
- **tags**: The tags of the volume, assigned by referencing the input variable volume_tags

### 5. Create a V3 Volume Snapshot

Add the following script to the TF file (such as main.tf) to create a V3 volume snapshot:
...
**Parameter description**:
- **region**: The region where the resource is located, assigned by referencing the input variable region_name
- **volume_id**: The ID of the volume to which the snapshot belongs, assigned by referencing the volume ID created in the previous step
