...
Data Replication Service (DRS) is a one-stop data replication service provided by Huawei Cloud, supporting scenarios such as database cloud migration, database migration, real-time database synchronization, and database disaster recovery. During the running of migration or synchronization jobs, the job's running logs, error logs, and performance metrics are critical for troubleshooting and O&M monitoring.

This best practice will introduce how to use Terraform to configure Log Tank Service (LTS) for a DRS job, collecting the DRS job logs into an LTS log group and log stream to achieve centralized log storage, query, and analysis.
...
- **subnet cidr**: The subnet CIDR block, automatically derived from the VPC CIDR block via the cidrsubnet function when the input variable subnet_cidr is empty
- **gateway_ip**: The subnet gateway IP, automatically derived via the cidrhost function when the input variable gateway_ip is empty
...
```hcl
# Create security group and rule resources in the specified region (if the region parameter is omitted, it inherits the region specified in the current provider block)
...
```hcl
# Query availability zones and RDS flavors data sources in the specified region (if the region parameter is omitted, it inherits the region specified in the current provider block)
...
```hcl
# Create source and destination RDS instance resources in the specified region (if the region parameter is omitted, it inherits the region specified in the current provider block)
...
- **flavor**: The instance flavor, assigned by referencing the input variable rds_flavor, and automatically uses the queried flavor when empty
- **security_group_id**: The security group ID, assigned by referencing the ID of the security group resource
- **subnet_id**: The subnet ID, assigned by referencing the ID of the subnet resource
- **vpc_id**: The VPC ID, assigned by referencing the ID of the VPC resource
- **fixed_ip**: The instance private IP, the source references the input variable source_rds_fixed_ip and the destination references the input variable dest_rds_fixed_ip
- **ha_replication_mode**: The HA replication mode, set to semisync
- **availability_zone**: The availability zone list, assigned by referencing the availability zones data source
- **db.password**: The database password, assigned by referencing the input variable db_password
- **volume**: The storage configuration, type is CLOUDSSD and size is 40GB
...
```hcl
# Create a DRS migration job resource in the specified region (if the region parameter is omitted, it inherits the region specified in the current provider block)
variable "job_name" {
...
- **engine_type**: The engine type, set to mysql
- **direction**: The migration direction, set to up (on-cloud)
...
- **force_destroy**: Whether to force destroy, set to true to allow deletion while the job is running
- **source_db**: The source database information, the IP references the private IP of the source RDS instance and the password is assigned by referencing the input variable db_password
- **destination_db**: The destination database information, assigned by referencing the ID, private IP, and subnet ID of the destination RDS instance
- **lifecycle.ignore_changes**: Ignores changes to the password, force_destroy, and action fields
...
```hcl
# Create LTS log group and log stream resources in the specified region (if the region parameter is omitted, it inherits the region specified in the current provider block)
...
- **ttl_in_days**: The log retention period in days, assigned by referencing the input variable lts_ttl_in_days
