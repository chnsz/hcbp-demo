...
Data Warehouse Service (DWS) is an enterprise-grade cloud data warehouse service provided by Huawei Cloud, supporting online analytical processing of massive data. During daily operations, key events of a cluster, such as scaling, restart, and failures, need to be perceived in a timely manner so that operators can respond quickly and ensure business continuity and stability.

This best practice will introduce how to use Terraform to automatically deploy a DWS event subscription, including the creation of a VPC, subnet, security group, DWS cluster, SMN topic and subscription, and the event subscription, helping you quickly build DWS cluster event notification capabilities using Infrastructure as Code (IaC).
...
- **count**: Creates the data source when the input variable availability_zone is empty, otherwise it is not created
...
- **enterprise_project_id**: Assigned by referencing the input variable enterprise_project_id, using the default enterprise project when empty
...
- **cidr**: Assigned by referencing the input variable subnet_cidr, automatically calculated based on the VPC CIDR when empty
- **gateway_ip**: Assigned by referencing the input variable subnet_gateway_ip, automatically calculated based on the VPC CIDR when empty
...
- **delete_default_rules**: Assigned by referencing the input variable security_group_delete_default_rules, controlling whether to delete the default rules of the security group
- **enterprise_project_id**: Assigned by referencing the input variable enterprise_project_id, using the default enterprise project when empty
...
**Parameter Description**:
- **count**: Creates the data source when the input variable cluster_node_type or cluster_version is empty, otherwise it is not created
- **availability_zone**: Assigned by referencing the input variable availability_zone, using the first availability zone from the availability zone list when empty
...
- **node_type**: Assigned by referencing the input variable cluster_node_type, using the flavor ID queried from the flavor data source when empty
...
- **version**: Assigned by referencing the input variable cluster_version, using the version queried from the flavor data source when empty
- **vpc_id**: Assigned by referencing the ID of the resource huaweicloud_vpc.test
...
**Parameter Description**:
- **name**: Assigned by referencing the input variable smn_topic_name
- **display_name**: Assigned by referencing the input variable smn_topic_display_name
...
**Parameter Description**:
- **topic_urn**: Assigned by referencing the ID of the resource huaweicloud_smn_topic.test
- **endpoint**: Assigned by referencing the input variable smn_subscription_endpoint
- **protocol**: Assigned by referencing the input variable smn_subscription_protocol
- **remark**: Assigned by referencing the input variable smn_subscription_remark
...
**Parameter Description**:
- **name**: Assigned by referencing the input variable event_subscription_name
- **enable**: Whether to enable the event subscription, where "1" means enabled
- **notification_target**: Assigned by referencing the ID of the resource huaweicloud_smn_topic.test
- **notification_target_name**: Assigned by referencing the name of the resource huaweicloud_smn_topic.test
- **notification_target_type**: The notification target type, currently only SMN is supported
- **category**: Assigned by referencing the input variable event_category
- **severity**: Assigned by referencing the input variable event_severity
- **source_type**: Assigned by referencing the input variable event_source_type
- **time_zone**: Assigned by referencing the input variable time_zone
