...
云搜索服务（Cloud Search Service，简称CSS）是华为云基于Elasticsearch、OpenSearch构建的完全托管式在线分布式搜索服务，支持结构化、非结构化文本及AI向量的高效检索与分析。CSS提供AI Ops智能运维能力，可对集群进行健康巡检和异常检测，帮助用户及时发现并处理潜在问题，提升集群稳定性。

本最佳实践将介绍如何使用Terraform自动化部署CSS AI Ops设置，包括创建VPC、子网、安全组、CSS集群，以及配置AI Ops巡检设置。
...
- [CSS AI Ops设置资源（huaweicloud_css_ai_ops_setting）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/css_ai_ops_setting)
...
在TF文件（如main.tf）中添加以下脚本以告知Terraform创建CSS AI Ops设置资源：
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建CSS AI Ops设置资源
...
**参数说明**：
- **cluster_id**：CSS集群ID，引用前面创建的CSS集群资源（huaweicloud_css_cluster.test）的ID
- **check_type**：AI Ops巡检类型，通过引用输入变量ai_ops_check_type进行赋值，默认为"full_detection"
- **period**：AI Ops巡检周期，通过引用输入变量ai_ops_period进行赋值，默认为"12:00 GMT+08:00"
- **check_items**：AI Ops巡检项列表，通过引用输入变量ai_ops_check_items进行赋值，默认为null
...
1. 命令行参数：`terraform apply -var="cluster_name=tf_test_ai_ops" -var="vpc_name=tf_test_ai_ops"`
2. 环境变量：`export TF_VAR_cluster_name=tf_test_ai_ops`
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建CSS AI Ops设置
4. 运行 `terraform show` 查看已创建的CSS AI Ops设置

## 参考信息

- [华为云CSS产品文档](https://support.huaweicloud.com/css/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [CSS AI Ops设置最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/css/ai-ops-setting)
