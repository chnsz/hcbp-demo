...
云搜索服务（Cloud Search Service，简称CSS）是华为云基于Elasticsearch、OpenSearch构建的完全托管式在线分布式搜索服务，支持结构化、非结构化文本及AI向量的高效检索与分析。CSS Logstash集群基于开源Logstash提供数据收集、转换、清洗、解析等能力，可帮助用户构建稳定可靠的数据采集与处理链路。

本最佳实践将介绍如何使用Terraform自动化部署一个CSS Logstash集群，包括创建VPC、子网、安全组，以及配置节点规格、存储和计费方式等参数。
...
- [CSS Logstash集群资源（huaweicloud_css_logstash_cluster）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/css_logstash_cluster)
...
### 2. 通过数据源查询CSS Logstash集群创建所需的可用区

在TF文件（如main.tf）中添加以下脚本以告知Terraform进行一次数据源查询，其查询结果用于创建CSS Logstash集群：
...
# 获取指定region（region参数缺省时默认继承当前provider块中所指定的region）下所有的可用区信息，用于创建CSS Logstash集群
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建VPC资源，用于部署CSS Logstash集群
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建VPC子网资源，用于部署CSS Logstash集群
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建安全组资源，用于部署CSS Logstash集群
...
### 6. 通过数据源查询CSS Logstash集群创建所需的规格
...
# 获取指定region（region参数缺省时默认继承当前provider块中所指定的region）下所有的CSS规格信息，用于创建CSS Logstash集群
...
在TF文件（如main.tf）中添加以下脚本以告知Terraform创建CSS Logstash集群资源：
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建CSS Logstash集群资源
...
**参数说明**：
- **name**：CSS Logstash集群名称，通过引用输入变量cluster_name进行赋值
- **engine_version**：CSS Logstash集群引擎版本，通过引用输入变量cluster_engine_version进行赋值，默认为"7.10.0"
- **availability_zone**：CSS Logstash集群所在可用区，当availability_zone为空时根据可用区列表查询数据源（data.huaweicloud_availability_zones）的返回结果进行赋值，否则通过引用输入变量availability_zone进行赋值
- **vpc_id**：CSS Logstash集群所属VPC的ID，引用前面创建的VPC资源（huaweicloud_vpc.test）的ID
- **subnet_id**：CSS Logstash集群所属子网的ID，引用前面创建的VPC子网资源（huaweicloud_vpc_subnet.test）的ID
- **security_group_id**：CSS Logstash集群关联的安全组ID，引用前面创建的安全组资源（huaweicloud_networking_secgroup.test）的ID
- **node_config**：CSS Logstash集群节点配置块
  - **flavor**：CSS Logstash集群节点规格，当cluster_flavor为空时根据CSS规格列表查询数据源（data.huaweicloud_css_flavors）的返回结果进行赋值，否则通过引用输入变量cluster_flavor进行赋值
  - **instance_number**：CSS Logstash集群节点数量，通过引用输入变量cluster_instance_number进行赋值，默认为1
  - **volume**：节点存储配置块
...
1. 命令行参数：`terraform apply -var="cluster_name=tf_test_logstash_cluster" -var="vpc_name=tf_test_logstash_cluster"`
2. 环境变量：`export TF_VAR_cluster_name=tf_test_logstash_cluster`
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建CSS Logstash集群
4. 运行 `terraform show` 查看已创建的CSS Logstash集群

## 参考信息

- [华为云CSS产品文档](https://support.huaweicloud.com/css/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [CSS Logstash集群最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/css/logstash-cluster)
