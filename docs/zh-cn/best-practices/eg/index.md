...
* [部署自定义事件订阅](custom.md) - 介绍如何使用Terraform自动化部署自定义事件订阅，包括自定义事件通道创建、自定义事件源配置、EG连接查询和事件订阅管理。
...
* [部署事件订阅（OBS事件源、Kafka事件目标）](obs_to_kafka.md) - 介绍如何使用Terraform自动化部署事件订阅（OBS事件源、Kafka事件目标），包括可用区列表（data.）、Kafka规格列表（data.）、事件通道列表（data.）、VPC创建和子网配置。
