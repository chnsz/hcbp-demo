...
云硬盘（Elastic Volume Service，EVS）是华为云提供的高性能、高可靠、可扩展的块存储服务，为弹性云服务器（ECS）等计算资源提供持久化存储。云硬盘快照可以在指定时间点对云硬盘数据进行备份，当发生误删除、数据损坏或业务故障时，可通过快照快速恢复数据，保障业务连续性。

本最佳实践将介绍如何使用Terraform自动化部署EVS V3云硬盘与V3快照，包括可用区与镜像数据源查询、云硬盘创建以及基于该云硬盘的快照创建。
...
**参数说明**：
- **count**：当未指定镜像ID时创建该数据源，通过引用输入变量 volume_image_id 进行判断
...
- **volume_type**：云硬盘的类型，通过引用输入变量 volume_type 进行赋值，支持SATA、SAS、GPSSD、SSD、ESSD、GPSSD2、ESSD2
- **availability_zone**：云硬盘所在的可用区，未指定时默认使用可用区列表中的第一个可用区
- **description**：云硬盘的描述信息，通过引用输入变量 volume_description 进行赋值
- **image_id**：用于创建云硬盘的镜像ID，未指定时使用镜像列表中的第一个镜像
- **metadata**：云硬盘的元数据，通过引用输入变量 volume_metadata 进行赋值
- **multiattach**：是否为共享云硬盘，通过引用输入变量 volume_multiattach 进行赋值
- **name**：云硬盘的名称，通过引用输入变量 volume_name 进行赋值
- **size**：云硬盘的大小（GB），通过引用输入变量 volume_size 进行赋值
- **tags**：云硬盘的标签，通过引用输入变量 volume_tags 进行赋值

### 5. 创建V3云硬盘快照

在TF文件（如main.tf）中添加以下脚本以创建V3云硬盘快照：
...
**参数说明**：
- **region**：资源所在的区域，通过引用输入变量 region_name 进行赋值
- **volume_id**：快照所属的云硬盘ID，引用上一步创建的云硬盘ID进行赋值
