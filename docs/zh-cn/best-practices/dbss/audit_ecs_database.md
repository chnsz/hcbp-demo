...
数据库安全服务（Database Security Service，DBSS）提供数据库安全审计能力，采用旁路部署方式，支持对华为云上RDS以及ECS/BMS自建数据库进行审计。通过创建DBSS审计实例并添加ECS自建数据库，可实时记录数据库访问行为，形成细粒度审计报告，并对风险行为和攻击行为进行告警，帮助企业满足合规要求并保障数据资产安全。本最佳实践将介绍如何使用Terraform自动化部署DBSS审计实例并添加ECS自建数据库，包括创建VPC、子网、安全组、DBSS实例，以及配置自建数据库审计信息。
...
- [ECS自建数据库资源（huaweicloud_dbss_ecs_database）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/dbss_ecs_database)
...
**参数说明**：
- **count**：数据源的创建数，用于控制是否执行DBSS规格列表查询数据源，仅当 `var.instance_flavor` 为空时创建数据源（即执行规格列表查询）
...
在TF文件（如main.tf）中添加以下脚本以告知Terraform创建ECS自建数据库资源：
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建ECS自建数据库资源
...
- **name**：自建数据库名称，通过引用输入变量 `database_name` 进行赋值
- **type**：自建数据库类型，通过引用输入变量 `database_type` 进行赋值
- **version**：自建数据库版本，通过引用输入变量 `database_version` 进行赋值
- **ip**：自建数据库IP地址，通过引用输入变量 `database_ip_address` 进行赋值
- **port**：自建数据库端口，通过引用输入变量 `database_port` 进行赋值
- **os**：自建数据库操作系统，通过引用输入变量 `database_os` 进行赋值
- **charset**：自建数据库字符集，通过引用输入变量 `database_charset` 进行赋值

> 注意：ECS自建数据库依赖已创建的DBSS实例。请确保目标自建数据库网络可达，并按实际环境填写数据库IP等信息。
...
1. 命令行参数：`terraform apply -var="vpc_name=tf_test_audit" -var="database_ip_address=YourDatabaseIPAddress"`
2. 环境变量：`export TF_VAR_instance_name=tf_test_audit`
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建审计ECS数据库相关资源
4. 运行 `terraform show` 查看已创建的审计ECS数据库相关资源

## 参考信息

- [华为云DBSS产品文档](https://support.huaweicloud.com/dbss/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [DBSS审计ECS数据库最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/dbss/audit-ecs-database)
