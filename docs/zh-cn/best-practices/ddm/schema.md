...
分布式数据库中间件（Distributed Database Middleware，简称DDM）是一款兼容MySQL协议的分布式关系型数据库中间件，专注于解决数据库分布式扩展问题，突破传统数据库的容量和性能瓶颈，实现海量数据高并发访问。逻辑库（Schema）是DDM对外提供的逻辑数据库单元，通过关联RDS等数据节点并配置分片模式，可实现分库分表与数据路由。本最佳实践将介绍如何使用Terraform自动化部署DDM逻辑库，包括创建VPC、子网、安全组，部署RDS数据节点与DDM实例，并创建关联数据节点的逻辑库。
...
- [DDM逻辑库资源（huaweicloud_ddm_schema）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/ddm_schema)
...
在TF文件（如main.tf）中添加以下脚本以告知Terraform进行一次数据源查询，其查询结果用于创建RDS实例和DDM实例：
...
# 获取指定region（region参数缺省时默认继承当前provider块中所指定的region）下所有的可用区信息，用于创建RDS实例和DDM实例
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建VPC资源，用于部署RDS实例和DDM实例
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建VPC子网资源，用于部署RDS实例和DDM实例
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建安全组资源，用于部署RDS实例和DDM实例
...
# 创建随机密码资源，用于在未指定RDS实例密码时生成数据库密码
...
**参数说明**：
- **count**：资源的创建数，用于控制是否创建随机密码资源，仅当 `var.rds_instance_password` 为空时创建该资源
...
**参数说明**：
- **count**：数据源的创建数，用于控制是否执行RDS规格列表查询数据源，仅当 `var.instance_flavor` 为空时创建数据源（即执行规格列表查询）
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建RDS实例资源，用作DDM逻辑库的数据节点
...
- **availability_zone**：RDS实例可用区列表，当输入变量为空列表时根据可用区列表查询数据源的返回结果进行赋值，否则使用输入变量的值
...
- **db**：数据库配置
  ...
  - **port**：数据库端口，通过引用输入变量 `database_port` 进行赋值
  - **password**：数据库密码，当输入变量为空时根据随机密码资源的返回结果进行赋值，否则使用输入变量的值
- **volume**：磁盘配置
  - **type**：磁盘类型，通过引用输入变量 `volume_type` 进行赋值
  - **size**：磁盘大小，通过引用输入变量 `volume_size` 进行赋值

> 注意：请妥善保管数据库密码等敏感信息，避免将其提交到版本控制系统。
...
**参数说明**：
- **name**：DDM实例名称，通过引用输入变量 `ddm_instance_name` 进行赋值
...
在TF文件（如main.tf）中添加以下脚本以告知Terraform创建DDM逻辑库资源：
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建DDM逻辑库资源
...
**参数说明**：
- **instance_id**：逻辑库所属DDM实例的ID，通过引用DDM实例资源的ID进行赋值
- **name**：逻辑库名称，通过引用输入变量 `schema_name` 进行赋值
- **shard_mode**：逻辑库分片模式，通过引用输入变量 `schema_shard_mode` 进行赋值
- **shard_number**：同一工作模式下的分片数量，通过引用输入变量 `schema_shard_number` 进行赋值
- **data_nodes**：逻辑库关联的数据节点配置
  - **id**：数据节点ID，通过引用RDS实例资源的ID进行赋值
  - **admin_user**：数据节点管理员用户名，本实践中固定为 `"root"`
  - **admin_password**：数据节点管理员密码，当输入变量为空时根据随机密码资源的返回结果进行赋值，否则使用输入变量的值
- **lifecycle.ignore_changes**：生命周期管理，忽略 `data_nodes` 的变更

> 注意：请妥善保管数据节点管理员密码等敏感信息，避免将其提交到版本控制系统。
...
1. 命令行参数：`terraform apply -var="vpc_name=tf_test_schema" -var="schema_name=tf_test_schema"`
2. 环境变量：`export TF_VAR_schema_name=tf_test_schema`
