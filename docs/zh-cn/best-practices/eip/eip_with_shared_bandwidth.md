...
共享带宽（Shared Bandwidth）是华为云提供的一种带宽复用能力，可以将多个弹性公网IP（EIP）加入同一份带宽资源中，实现带宽的共享与复用，从而降低公网带宽成本并提升带宽利用率。在实际业务中，当多台云服务器或多种云资源需要访问公网时，为每个EIP单独购买带宽往往会造成带宽闲置与成本浪费，而通过共享带宽可以按整体峰值统一计费，更加经济高效。

本最佳实践将介绍如何使用Terraform自动化部署共享带宽，并在该共享带宽上创建弹性公网IP，实现EIP与共享带宽的绑定管理。
...
在TF文件（如main.tf）中添加以下脚本以创建弹性公网IP并绑定到上一步创建的共享带宽：
...
- **bandwidth.share_type**：设置为WHOLE，表示该EIP使用共享带宽
- **bandwidth.id**：引用上一步创建的共享带宽的ID，实现EIP与共享带宽的绑定
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建共享带宽和弹性公网IP
4. 运行 `terraform show` 查看已创建的共享带宽和弹性公网IP

## 参考信息

- [华为云弹性公网IP产品文档](https://support.huaweicloud.com/eip/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [EIP绑定共享带宽最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/eip/eip-with-shared-bandwidth)
