...
* [部署AutoScaler插件](addon_autoscaler.md) - 介绍如何使用Terraform自动化部署一个CCE AutoScaler插件，包括CCE集群、插件模板和IAM项目的查询，以及CCE插件的创建。
...
* [使用现有OBS部署Kubernetes PVC](kubernetes_pvc_with_existing_obs.md) - 介绍如何使用Terraform自动化部署一个通过OBS管理PVC的完整方案，包括可用区、实例规格的查询，以及VPC、子网、弹性公网IP、CCE集群、节点、OBS桶等基础设施的创建，以及Kubernetes Secret、Persistent Volume、Persistent Volume Claim和Deployment的创建。
* [使用新OBS部署Kubernetes PVC](kubernetes_pvc_with_new_obs.md) - 介绍如何使用Terraform自动化部署一个通过新OBS管理PVC的完整方案，包括可用区、实例规格的查询，以及VPC、子网、弹性公网IP、CCE集群、节点等基础设施的创建，以及Kubernetes Secret、Persistent Volume Claim和Deployment的创建。与使用现有OBS桶的方式不同，本最佳实践通过PVC自动创建OBS桶和Persistent Volume，简化了部署流程。
