...
云容器引擎（Cloud Container Engine，CCE）是一个高可靠高性能的企业级容器管理服务，支持Kubernetes社区原生应用和工具。Kubernetes命名空间是Kubernetes中用于资源隔离和组织的逻辑分组机制，可以将集群中的资源（如Pod、Service、Deployment等）组织到不同的命名空间中，实现资源的逻辑隔离和管理。通过创建Kubernetes命名空间，可以为不同的应用、团队或环境提供独立的资源空间，提高资源管理的灵活性和安全性。本最佳实践将介绍如何使用Terraform自动化部署一个Kubernetes命名空间，包括可用区、实例规格的查询，以及VPC、子网、弹性公网IP、CCE集群、节点等基础设施的创建，以及Kubernetes命名空间的创建。
...
- [Kubernetes命名空间资源（kubernetes_namespace）](https://registry.terraform.io/providers/hashicorp/kubernetes/latest/docs/resources/namespace)
...
### 3. 通过数据源查询命名空间资源创建所需的可用区

在TF文件（如main.tf）中添加以下脚本以告知Terraform进行一次数据源查询，其查询结果用于创建命名空间相关资源：
...
# 获取指定region（region参数缺省时默认继承当前provider块中所指定的region）下所有的可用区信息，用于创建命名空间相关资源
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建VPC资源，用于为命名空间提供网络环境
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建VPC子网资源，用于为命名空间提供网络环境
...
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建弹性公网IP资源，用于为命名空间提供公网访问能力
...
# 获取指定region（region参数缺省时默认继承当前provider块中所指定的region）下所有符合特定条件的实例规格信息，用于创建命名空间相关资源
...
### 11. 创建Kubernetes命名空间资源

在TF文件中添加以下脚本以告知Terraform创建Kubernetes命名空间资源：

```hcl
variable "namespace_name" {
  description = "Kubernetes命名空间的名称"
  type        = string
}

# 在CCE集群中创建Kubernetes命名空间资源，用于组织和管理Kubernetes资源
...
**参数说明**：
- **metadata**：命名空间的元数据配置块
  - **name**：命名空间的名称，通过引用输入变量 `namespace_name` 进行赋值
- **depends_on**：显式依赖关系，确保CCE节点资源在命名空间之前创建完成

> 注意：Kubernetes命名空间是Kubernetes中的逻辑分组机制，用于将集群中的资源组织到不同的命名空间中。命名空间的创建依赖于Kubernetes provider能够成功连接到CCE集群，因此需要确保集群和节点已经创建完成。
...
1. 运行 `terraform init` 初始化环境（这将下载Kubernetes provider）
2. 运行 `terraform plan` 查看资源创建计划
3. 确认资源计划无误后，运行 `terraform apply` 开始创建Kubernetes命名空间
4. 运行 `terraform show` 查看已创建的Kubernetes命名空间
...
- [CCE Kubernetes命名空间最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/cce/kubenetes/namespace)
