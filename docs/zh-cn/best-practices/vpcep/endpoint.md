...
VPC终端节点（VPC Endpoint，VPCEP）是华为云提供的VPC内资源互访服务，支持在VPC内创建终端节点和终端节点服务，实现VPC内资源的私网访问。终端节点是VPCEP服务的核心功能，用于在VPC内创建终端节点，连接到终端节点服务，实现跨VPC的私网访问。通过终端节点，可以访问其他VPC中发布的终端节点服务，实现安全的私网通信，避免通过公网访问，提高访问安全性和稳定性。本最佳实践将介绍如何使用Terraform自动化部署终端节点，包括可用区查询、ECS规格查询、镜像查询、VPC、子网、安全组、ECS实例、终端节点服务和终端节点的创建。
...
> 注意：终端节点需要依赖终端节点服务，终端节点服务需要依赖ECS实例，ECS实例需要依赖VPC、子网、安全组、可用区、规格和镜像资源。终端节点通过关联终端节点服务，实现跨VPC的私网访问。
...
> 注意：终端节点用于连接到终端节点服务，实现跨VPC的私网访问。创建终端节点后，可以通过终端节点访问终端节点服务提供的服务资源。
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建终端节点及相关资源
4. 运行 `terraform show` 查看已创建的终端节点

## 参考信息

- [华为云VPCEP产品文档](https://support.huaweicloud.com/vpcep/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [终端节点最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/vpcep/endpoint)
