...
VPC终端节点（VPC Endpoint，VPCEP）是华为云提供的VPC内资源互访服务，支持在VPC内创建终端节点和终端节点服务，实现VPC内资源的私网访问。终端节点审批是VPCEP服务的重要功能，用于审批终端节点连接到终端节点服务的请求。当终端节点尝试连接到需要审批的终端节点服务时，服务提供方需要审批该连接请求，确保只有授权的终端节点可以访问服务资源。通过终端节点审批，可以实现访问控制和权限管理，提高服务资源的安全性。本最佳实践将介绍如何使用Terraform自动化部署终端节点审批，包括可用区查询、ECS规格查询、镜像查询、VPC、子网、安全组、ECS实例、终端节点服务、终端节点和终端节点审批的创建。
...
- [终端节点资源（huaweicloud_vpcep_endpoint）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/vpcep_endpoint)
- [终端节点审批资源（huaweicloud_vpcep_approval）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/vpcep_approval)
...
> 注意：终端节点审批需要依赖终端节点和终端节点服务。终端节点需要依赖终端节点服务，终端节点服务需要依赖ECS实例，ECS实例需要依赖VPC、子网、安全组、可用区、规格和镜像资源。终端节点审批用于审批终端节点连接到终端节点服务的请求。
...
**参数说明**：
- **service_id**：终端节点服务ID，通过引用终端节点服务资源的ID进行赋值
- **vpc_id**：终端节点所属的VPC ID，通过引用VPC资源的ID进行赋值
- **network_id**：终端节点所属的子网ID，通过引用子网资源的ID进行赋值

### 10. 创建终端节点审批

在TF文件（如main.tf）中添加以下脚本以创建终端节点审批：
...
**参数说明**：
- **service_id**：终端节点服务ID，通过引用终端节点服务资源的ID进行赋值
- **endpoints**：终端节点ID列表，通过引用终端节点资源的ID进行赋值，用于指定需要审批的终端节点

> 注意：终端节点审批用于审批终端节点连接到终端节点服务的请求。当终端节点尝试连接到需要审批的终端节点服务时，服务提供方需要通过审批操作来允许或拒绝该连接请求，确保只有授权的终端节点可以访问服务资源。
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建终端节点审批及相关资源
4. 运行 `terraform show` 查看已创建的终端节点审批

## 参考信息

- [华为云VPCEP产品文档](https://support.huaweicloud.com/vpcep/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [终端节点审批最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/vpcep/approval)
