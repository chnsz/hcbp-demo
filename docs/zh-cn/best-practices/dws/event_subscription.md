...
数据仓库服务（Data Warehouse Service，DWS）是华为云提供的在线联机分析处理（OLAP）企业级数据仓库服务，支持海量数据的快速查询与分析。DWS 在集群运行过程中会产生集群管理、安全、容灾等多类事件，运维人员需要及时感知这些事件并做出响应。

本最佳实践将介绍如何使用Terraform自动化部署DWS事件订阅，包括VPC、子网、安全组、DWS集群、SMN主题与订阅以及事件订阅的创建，帮助您及时接收DWS集群的事件通知，提升运维效率。
...
- [DWS集群规格列表（data.huaweicloud_dws_flavors）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/data-sources/dws_flavors)
...
- **count**：当输入变量 availability_zone 为空时，才查询可用区列表，用于为DWS集群自动选择可用区
...
- **cidr**：通过引用输入变量 vpc_cidr 进行赋值，指定VPC的网段
- **enterprise_project_id**：通过引用输入变量 enterprise_project_id 进行赋值，当变量为空时使用默认企业项目
...
- **cidr**：通过引用输入变量 subnet_cidr 进行赋值，当变量为空时基于VPC网段自动划分子网网段
- **gateway_ip**：通过引用输入变量 subnet_gateway_ip 进行赋值，当变量为空时基于子网网段自动计算网关IP
...
- **delete_default_rules**：通过引用输入变量 security_group_delete_default_rules 进行赋值，指定是否删除安全组默认规则
- **enterprise_project_id**：通过引用输入变量 enterprise_project_id 进行赋值，当变量为空时使用默认企业项目

### 6. 查询DWS集群规格列表

在TF文件中添加以下脚本以查询DWS集群规格：

```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下查询DWS集群规格列表
...
- **count**：当输入变量 cluster_node_type 或 cluster_version 为空时，才查询DWS集群规格列表
- **availability_zone**：通过引用输入变量 availability_zone 或可用区列表进行赋值，指定规格查询的可用区
- **vcpus**：通过引用输入变量 cluster_vcpus 进行赋值，指定规格的vCPU数量
- **memory**：通过引用输入变量 cluster_memory 进行赋值，指定规格的内存大小
- **datastore_type**：通过引用输入变量 cluster_datastore_type 进行赋值，指定规格的数据存储类型

### 7. 创建DWS集群

在TF文件中添加以下脚本以创建DWS集群：
...
- **name**：通过引用输入变量 cluster_name 进行赋值，指定DWS集群的名称
- **node_type**：通过引用输入变量 cluster_node_type 或规格列表进行赋值，指定集群的节点规格
- **number_of_node**：通过引用输入变量 cluster_number_of_node 进行赋值，指定集群的节点数量
- **number_of_cn**：通过引用输入变量 cluster_number_of_cn 进行赋值，指定集群的CN节点数量
- **version**：通过引用输入变量 cluster_version 或规格列表进行赋值，指定集群的版本
- **vpc_id**：通过引用 huaweicloud_vpc.test.id 进行赋值，指定集群所属的VPC
- **network_id**：通过引用 huaweicloud_vpc_subnet.test.id 进行赋值，指定集群所属的子网
- **security_group_id**：通过引用 huaweicloud_networking_secgroup.test.id 进行赋值，指定集群所属的安全组
- **availability_zone**：通过引用输入变量 availability_zone 或可用区列表进行赋值，指定集群的可用区
- **user_name**：通过引用输入变量 cluster_admin_user_name 进行赋值，指定集群的管理员用户名
- **user_pwd**：通过引用输入变量 cluster_admin_user_pwd 进行赋值，指定集群的管理员密码
- **enterprise_project_id**：通过引用输入变量 enterprise_project_id 进行赋值，当变量为空时使用默认企业项目
- **volume.type**：通过引用输入变量 cluster_volume_type 进行赋值，指定集群的磁盘类型
- **volume.capacity**：通过引用输入变量 cluster_volume_capacity 进行赋值，指定集群的磁盘容量

### 8. 创建SMN主题

在TF文件中添加以下脚本以创建SMN主题：
...
- **name**：通过引用输入变量 smn_topic_name 进行赋值，指定SMN主题的名称
- **display_name**：通过引用输入变量 smn_topic_display_name 进行赋值，指定SMN主题的显示名称

### 9. 创建SMN订阅

在TF文件中添加以下脚本以创建SMN订阅：
...
- **topic_urn**：通过引用 huaweicloud_smn_topic.test.id 进行赋值，指定订阅所属的SMN主题
- **endpoint**：通过引用输入变量 smn_subscription_endpoint 进行赋值，指定消息接收终端
- **protocol**：通过引用输入变量 smn_subscription_protocol 进行赋值，指定消息接收协议
- **remark**：通过引用输入变量 smn_subscription_remark 进行赋值，指定订阅的备注信息

### 10. 创建DWS事件订阅

在TF文件中添加以下脚本以创建DWS事件订阅：
...
- **name**：通过引用输入变量 event_subscription_name 进行赋值，指定事件订阅的名称
- **enable**：指定是否启用事件订阅，取值为1表示启用
- **notification_target**：通过引用 huaweicloud_smn_topic.test.id 进行赋值，指定事件通知的目标
- **notification_target_name**：通过引用 huaweicloud_smn_topic.test.name 进行赋值，指定事件通知目标的名称
- **notification_target_type**：指定事件通知目标的类型，当前仅支持SMN
- **category**：通过引用输入变量 event_category 进行赋值，指定订阅的事件类别
- **severity**：通过引用输入变量 event_severity 进行赋值，指定订阅的事件级别
- **source_type**：通过引用输入变量 event_source_type 进行赋值，指定订阅的事件源类型
- **time_zone**：通过引用输入变量 time_zone 进行赋值，指定事件订阅的时区
