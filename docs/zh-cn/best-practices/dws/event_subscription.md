...
数据仓库服务（Data Warehouse Service，DWS）是华为云提供的在线联机分析处理（OLAP）数据库服务，支持海量数据的快速查询与分析。在实际运维中，集群的扩容、缩容、重启、故障等关键事件需要被及时感知，以便运维人员快速响应，保障业务的连续性与稳定性。

本最佳实践将介绍如何使用Terraform自动化部署DWS事件订阅，包括VPC、子网、安全组、DWS集群、SMN主题与订阅以及DWS事件订阅的创建，帮助您通过基础设施即代码（IaC）的方式快速构建DWS集群的事件通知能力。
...
- **enterprise_project_id**：企业项目ID，通过引用输入变量 enterprise_project_id 进行赋值，当取值为空字符串时表示使用默认企业项目
...
- **cidr**：子网的网段，通过引用输入变量 subnet_cidr 进行赋值，当取值为空字符串时基于VPC网段自动划分子网
- **gateway_ip**：子网的网关IP，通过引用输入变量 subnet_gateway_ip 进行赋值，当取值为空字符串时基于子网网段自动计算网关IP
...
- **delete_default_rules**：是否删除安全组的默认规则，通过引用输入变量 security_group_delete_default_rules 进行赋值
- **enterprise_project_id**：企业项目ID，通过引用输入变量 enterprise_project_id 进行赋值，当取值为空字符串时表示使用默认企业项目
...
**参数说明**：
- **count**：数据源的创建数量，当输入变量 availability_zone 为空字符串时创建该数据源，用于自动获取当前region下的可用区列表
...
```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下查询DWS集群规格数据源
...
**参数说明**：
- **count**：数据源的创建数量，当输入变量 cluster_node_type 或 cluster_version 为空字符串时创建该数据源，用于自动获取匹配的集群规格
- **availability_zone**：可用区，通过引用输入变量 availability_zone 进行赋值，当取值为空字符串时使用可用区列表数据源返回的第一个可用区
- **vcpus**：集群节点的vCPU数量，通过引用输入变量 cluster_vcpus 进行赋值
- **memory**：集群节点的内存大小，通过引用输入变量 cluster_memory 进行赋值
- **datastore_type**：集群的数据存储类型，通过引用输入变量 cluster_datastore_type 进行赋值
...
```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建DWS集群资源
...
**参数说明**：
- **name**：DWS集群名称，通过引用输入变量 cluster_name 进行赋值
- **node_type**：集群节点规格，通过引用输入变量 cluster_node_type 进行赋值，当取值为空字符串时使用DWS集群规格数据源返回的规格ID
- **number_of_node**：集群节点数量，通过引用输入变量 cluster_number_of_node 进行赋值
- **number_of_cn**：集群CN节点数量，通过引用输入变量 cluster_number_of_cn 进行赋值
- **version**：集群版本，通过引用输入变量 cluster_version 进行赋值，当取值为空字符串时使用DWS集群规格数据源返回的版本
- **vpc_id**：集群所属的VPC ID，引用前一步创建的VPC资源的ID
- **network_id**：集群所属的子网ID，引用前一步创建的子网资源的ID
- **security_group_id**：集群所属的安全组ID，引用前一步创建的安全组资源的ID
- **availability_zone**：集群的可用区，通过引用输入变量 availability_zone 进行赋值，当取值为空字符串时使用可用区列表数据源返回的第一个可用区
- **user_name**：集群管理员用户名，通过引用输入变量 cluster_admin_user_name 进行赋值
- **user_pwd**：集群管理员密码，通过引用输入变量 cluster_admin_user_pwd 进行赋值
- **enterprise_project_id**：企业项目ID，通过引用输入变量 enterprise_project_id 进行赋值，当取值为空字符串时表示使用默认企业项目
- **volume.type**：集群数据盘类型，通过引用输入变量 cluster_volume_type 进行赋值
- **volume.capacity**：集群数据盘容量，通过引用输入变量 cluster_volume_capacity 进行赋值
...
- **display_name**：SMN主题显示名称，通过引用输入变量 smn_topic_display_name 进行赋值
...
**参数说明**：
- **topic_urn**：订阅所属的SMN主题URN，引用前一步创建的SMN主题资源的ID
- **endpoint**：消息接收终端，通过引用输入变量 smn_subscription_endpoint 进行赋值
- **protocol**：消息接收协议，通过引用输入变量 smn_subscription_protocol 进行赋值
- **remark**：订阅备注信息，通过引用输入变量 smn_subscription_remark 进行赋值
...
```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建DWS事件订阅资源
...
**参数说明**：
- **name**：事件订阅名称，通过引用输入变量 event_subscription_name 进行赋值
- **enable**：是否启用事件订阅，取值为"1"表示启用
- **notification_target**：通知目标，引用前一步创建的SMN主题资源的ID
- **notification_target_name**：通知目标名称，引用前一步创建的SMN主题资源的名称
- **notification_target_type**：通知目标类型，当前仅支持"SMN"
- **category**：订阅的事件类别，通过引用输入变量 event_category 进行赋值
- **severity**：订阅的事件级别，通过引用输入变量 event_severity 进行赋值
- **source_type**：订阅的事件源类型，通过引用输入变量 event_source_type 进行赋值
- **time_zone**：事件订阅的时区，通过引用输入变量 time_zone 进行赋值
