...
企业交换机（Enterprise Switch，ESW）是华为云提供的高性能、高可用的企业级网络交换服务，支持大二层网络互通，实现跨可用区的二层网络连接。通过ESW连接，可以将VPC子网与ESW实例进行连接，实现跨可用区的二层网络互通。本最佳实践将介绍如何使用Terraform自动化部署ESW连接，包括VPC、子网、ESW实例和ESW连接的创建。
...
完成以上脚本配置后，执行以下步骤来创建ESW连接：

1. 运行 `terraform init` 初始化环境
2. 运行 `terraform plan` 查看资源创建计划
3. 确认资源计划无误后，运行 `terraform apply` 开始创建VPC、子网、ESW实例和ESW连接
4. 运行 `terraform show` 查看已创建的ESW连接详情

> 注意：ESW实例创建后需要等待实例状态变为可用，才能继续创建ESW连接。确保VPC和子网已正确创建，并且子网CIDR配置正确。ESW连接创建后，可以实现跨可用区的二层网络互通。

## 参考信息

- [华为云ESW产品文档](https://support.huaweicloud.com/esw/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [连接最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/esw/connection)
