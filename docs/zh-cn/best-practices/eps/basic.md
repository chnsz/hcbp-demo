...
企业项目管理服务（Enterprise Project Management Service，EPS）是华为云提供的企业级资源管理服务，支持企业以项目为维度对云资源进行分组、授权和成本核算，实现资源的分域隔离与精细化管理。

本最佳实践将介绍如何使用Terraform自动化部署一个企业项目，包括项目名称、描述、类型、启用状态以及销毁行为的配置，帮助您快速构建企业级资源管理的基础单元。
...
- **type**：企业项目类型，通过引用输入变量 enterprise_project_type 进行赋值，可选值为 poc 和 prod
- **enable**：是否启用企业项目，通过引用输入变量 enterprise_project_enable 进行赋值
- **skip_disable_on_destroy**：销毁时是否跳过禁用操作，通过引用输入变量 skip_disable_on_destroy 进行赋值
