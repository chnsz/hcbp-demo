...
分布式缓存服务（DCS）是华为云提供的高性能、高可用的内存数据库服务，支持Redis等主流缓存引擎。随着业务运行，Redis实例中会积累大量已过期的Key，这些Key若长期未清理，会占用内存资源并影响实例性能。

本最佳实践将介绍如何使用Terraform自动化部署DCS Redis实例并发起过期Key扫描，包括VPC创建、子网配置、实例创建以及过期Key扫描任务的触发，帮助您及时掌握实例中过期Key的数量，为后续的内存优化和运维工作提供依据。
...
- [DCS实例过期Key扫描（huaweicloud_dcs_instance_expired_key_scan）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/dcs_instance_expired_key_scan)
...
**参数说明**：
- **vpc_id**：通过引用 huaweicloud_vpc.test.id 进行赋值，即上一步创建的VPC的ID
- **name**：通过引用输入变量 subnet_name 进行赋值
- **cidr**：通过引用输入变量 subnet_cidr 进行赋值，若为空则基于VPC的CIDR自动划分子网网段
...
**参数说明**：
- **count**：当输入变量 availability_zone 为空时查询可用分区列表，否则不查询
- **engine**：DCS产品规格的缓存引擎，固定为 Redis
- **capacity**：通过引用输入变量 instance_capacity 进行赋值，表示实例容量（单位：GB）
- **engine_version**：通过引用输入变量 instance_engine_version 进行赋值，表示实例引擎版本

### 5. 创建随机密码

在TF文件（如main.tf）中添加以下脚本以在未指定密码时自动生成随机密码：
...
**参数说明**：
- **count**：当输入变量 instance_password 为空时生成随机密码，否则不生成
- **length**：随机密码长度，固定为 12
...
- **password**：通过引用输入变量 instance_password 进行赋值，若为空则使用随机生成的密码
- **flavor**：通过引用输入变量 instance_flavor_id 进行赋值，若为空则使用查询到的DCS产品规格名称
- **availability_zones**：通过引用输入变量 availability_zone 进行赋值，若为空则使用查询到的第一个可用分区

### 7. 创建DCS实例过期Key扫描

在TF文件（如main.tf）中添加以下脚本以创建DCS实例过期Key扫描：

```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建DCS实例过期Key扫描资源
...
**参数说明**：
- **instance_id**：通过引用 huaweicloud_dcs_instance.test.id 进行赋值，即上一步创建的DCS Redis实例的ID
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建DCS Redis实例及过期Key扫描任务
4. 运行 `terraform show` 查看已创建的DCS Redis实例及过期Key扫描任务

## 参考信息

- [华为云DCS产品文档](https://support.huaweicloud.com/dcs/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [DCS Redis过期Key扫描最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/dcs/redis-expired-key-scan)
