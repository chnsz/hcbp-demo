...
分布式缓存服务（DCS）Redis实例在运行过程中会产生运行日志，用于记录实例的运行状态、慢查询、连接异常等关键信息。当实例出现性能波动或异常时，运维人员需要采集指定时间范围内的运行日志进行分析定位。

本最佳实践将介绍如何使用Terraform自动化部署DCS Redis高可用实例并采集其运行日志，包括VPC与子网创建、可用分区与产品规格查询、实例配置、节点查询以及运行日志采集任务的创建。
...
- [DCS Redis运行日志采集（huaweicloud_dcs_redis_run_log_collect）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/dcs_redis_run_log_collect)
...
- **cidr**：子网网段，当输入变量 subnet_cidr 为空时，自动从VPC网段中划分
- **gateway_ip**：子网网关IP，当输入变量 subnet_gateway_ip 为空时，自动从子网网段中计算
...
**参数说明**：
- **count**：当输入变量 availability_zone 为空时执行查询，否则使用用户指定的可用分区
...
**参数说明**：
- **count**：当输入变量 instance_flavor_id 为空时执行查询，否则使用用户指定的规格
- **cache_mode**：缓存实例类型，固定为 `ha`（主备）
- **cpu_architecture**：CPU架构，固定为 `x86_64`
...
**参数说明**：
- **count**：当输入变量 instance_password 为空时生成随机密码，否则使用用户指定的密码
- **length**：密码长度，固定为12
- **special**：是否包含特殊字符，固定为 `true`
- **override_special**：允许使用的特殊字符集合
- **min_upper**/**min_lower**/**min_numeric**/**min_special**：大写字母、小写字母、数字、特殊字符的最小数量
...
- **password**：实例密码，当输入变量 instance_password 为空时使用随机生成的密码
- **flavor**：实例规格，当输入变量 instance_flavor_id 为空时使用查询到的产品规格
- **availability_zones**：实例所属的可用分区，当输入变量 availability_zone 为空时使用查询到的可用分区
...
**参数说明**：
- **instance_id**：实例ID，引用前一步创建的DCS实例的ID

### 9. 创建Redis运行日志采集任务

在TF文件（如main.tf）中添加以下脚本以创建Redis运行日志采集任务：

```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下创建Redis运行日志采集资源
variable "log_type" {
...
**参数说明**：
- **instance_id**：实例ID，引用前一步创建的DCS实例的ID
- **log_type**：日志类型，通过引用输入变量 log_type 进行赋值，当前仅支持 `run`（Redis运行日志）
- **query_time**：查询时间偏移天数，通过引用输入变量 query_time 进行赋值，有效值为0、1、3、7，0表示当天日志
- **replication_id**：副本ID，引用查询到的实例节点的副本ID
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建Redis运行日志采集任务
4. 运行 `terraform show` 查看已创建的Redis运行日志采集任务

## 参考信息

- [华为云DCS产品文档](https://support.huaweicloud.com/dcs/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [DCS Redis运行日志采集最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/dcs/redis-run-log-collect)
