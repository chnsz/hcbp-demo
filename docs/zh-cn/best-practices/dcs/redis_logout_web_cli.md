...
分布式缓存服务（DCS）Redis实例提供Web CLI功能，允许用户通过浏览器直接连接并操作Redis实例。在完成运维操作后，及时登出Web CLI会话可以释放连接资源并降低安全风险。

本最佳实践将介绍如何使用Terraform自动化部署DCS Redis实例，并完成Web CLI的登录与登出操作，包括VPC创建、子网配置、实例创建、Web CLI登录以及Web CLI登出。
...
- [DCS Web CLI登出（huaweicloud_dcs_logout_web_cli）](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs/resources/dcs_logout_web_cli)
...
- **cidr**：子网的CIDR网段，当输入变量 subnet_cidr 为空时，自动从VPC网段中划分
- **gateway_ip**：子网的网关IP地址，当输入变量 subnet_gateway_ip 为空时，自动从子网网段中计算
...
**参数说明**：
- **count**：当输入变量 availability_zone 为空时查询可用分区列表，否则使用指定可用分区
- **cache_mode**：缓存模式，此处为 `ha`（主备模式）
...
**参数说明**：
- **count**：当输入变量 instance_password 为空时创建随机密码，否则使用指定密码
- **length**：密码长度，此处为12位
...
- **locals.dcs_password**：本地变量，当输入变量 instance_password 为空时使用随机生成的密码，否则使用指定密码
...
- **engine**：缓存引擎，此处为 `Redis`
- **engine_version**：引擎版本，通过引用输入变量 instance_engine_version 进行赋值
- **capacity**：实例容量，通过引用输入变量 instance_capacity 进行赋值
- **vpc_id**：实例所属的VPC ID，引用前一步创建的VPC资源的ID
- **subnet_id**：实例所属的子网ID，引用前一步创建的子网资源的ID
- **password**：实例密码，引用本地变量 dcs_password
- **flavor**：实例规格，当输入变量 instance_flavor_id 为空时使用查询到的产品规格名称
- **availability_zones**：实例所属的可用分区，当输入变量 availability_zone 为空时使用查询到的可用分区列表

### 7. 登录Web CLI

在TF文件（如main.tf）中添加以下脚本以登录Web CLI：

```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下登录Web CLI
...
**参数说明**：
- **instance_id**：DCS Redis实例ID，引用前一步创建的实例资源的ID
- **password**：实例密码，引用本地变量 dcs_password

### 8. 登出Web CLI

在TF文件（如main.tf）中添加以下脚本以登出Web CLI：

```hcl
# 在指定region（region参数缺省时默认继承当前provider块中所指定的region）下登出Web CLI
...
**参数说明**：
- **instance_id**：DCS Redis实例ID，引用前一步创建的实例资源的ID
- **client_id**：Web CLI客户端ID，引用前一步登录Web CLI资源的 client_id 属性，无需手动配置
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建DCS Redis实例并执行Web CLI登出
4. 运行 `terraform show` 查看已创建的DCS Redis实例及Web CLI登出资源

## 参考信息

- [华为云DCS产品文档](https://support.huaweicloud.com/dcs/index.html)
- [华为云Provider文档](https://registry.terraform.io/providers/huaweicloud/huaweicloud/latest/docs)
- [DCS Redis Web CLI登出最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/dcs/redis-logout-web-cli)
