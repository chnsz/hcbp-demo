...
分布式缓存服务（Distributed Cache Service，DCS）是华为云提供的高性能、高可用的内存数据库服务，支持Redis、Memcached等主流缓存引擎。在DCS实例的日常运维中，系统会在实例创建、扩容、配置变更、备份、恢复等操作时自动生成后台任务，这些任务记录在实例的后台任务列表中。

本最佳实践将介绍如何使用Terraform删除DCS Redis实例中指定的后台任务。该实践针对已存在的DCS实例和后台任务进行操作，不创建任何新的基础设施，适用于需要清理已完成或异常后台任务记录的场景。
