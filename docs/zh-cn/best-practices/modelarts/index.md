...
* [部署专属资源池](dedicated_resource_pool.md) - 介绍如何使用Terraform自动化部署ModelArts专属资源池，支持新建或复用ModelArts网络及SFS Turbo，包括VPC网络、SFS Turbo、ModelArts网络、工作空间及专属资源池的创建。
* [部署实时推理服务](deploy_real_time_service.md) - 介绍如何使用Terraform自动化部署ModelArts V2实时推理服务，包括V2资源池查询、实例组与单元配置、运行时配置、升级策略及日志配置。
* [部署微调训练作业](fine_tuning_training_job.md) - 介绍如何使用Terraform自动化部署ModelArts微调训练作业，包括工作空间、SMN主题及微调训练作业的创建，支持数据集输入、资产模型、输出模型和微调配置。
* [部署专属资源池Notebook](notebook_with_dedicated_resource_pool.md) - 介绍如何使用Terraform自动化部署ModelArts专属资源池及Notebook实例，包括VPC网络、SFS Turbo、ModelArts网络、工作空间、专属资源池、Notebook规格与镜像查询及存储挂载的创建。
