...
OBS KMS加密桶是OBS服务的重要安全功能，用于为存储桶提供服务端加密保护。通过KMS（Key Management Service）加密，企业可以确保存储在OBS中的数据得到加密保护，满足数据安全和合规要求。KMS加密支持多种加密算法，提供密钥管理和轮换功能，为企业提供企业级的数据安全保护。本最佳实践将介绍如何使用Terraform自动化部署OBS KMS加密桶，包括KMS密钥创建和OBS桶配置。
...
3. 确认资源计划无误后，运行 `terraform apply` 开始创建KMS加密桶
4. 运行 `terraform show` 查看已创建的KMS加密桶
