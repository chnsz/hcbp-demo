...
### 3. 通过数据源查询ECS实例资源创建所需的规格（data.huaweicloud_compute_flavors）
...
- **performance_type**：规格的类型，本例中使用"normal"标准类型
- **cpu_core_count**：CPU核心数，本例中设置为2核
- **memory_size**：内存大小（GB），本例中设置为4GB

### 4. 通过数据源查询ECS实例资源创建所需的镜像（data.huaweicloud_images_images）
...
- **visibility**：镜像的可见性，本例中使用"public"公共镜像
- **os**：镜像的操作系统类型，本例中使用"Ubuntu"操作系统

### 5. 创建VPC资源（huaweicloud_vpc）
...
### 6. 创建VPC子网资源（huaweicloud_vpc_subnet）
...
### 7. 创建安全组资源（huaweicloud_networking_secgroup）
...
### 8. 创建ECS实例（huaweicloud_compute_instance）
...
- **lifecycle**：生命周期配置块，用于控制资源的生命周期行为
  - **ignore_changes**：指定在后续应用中要忽略的属性变更，本例中忽略admin_pass的变更
...
- [ECS最佳实践源码参考](https://github.com/huaweicloud/terraform-provider-huaweicloud/blob/master/examples/ecs/basic) 